#!/usr/bin/env python3
"""
coupled_maxwell_plane_wave.py

2D coupled scalar + Maxwell gauge substrate with three modes:
  ...
  2) PHOTON_NOISE mode (--mode photon_noise):
       ...
       - Free-photon vacuum soup.

  3) PHOTON_PLANE mode (--mode photon_plane):
       - psi = 0, q = 0, g_defrag = 0.
       - Gauge potentials seeded with a single transverse plane wave:
           A(x,y,0) = amp_plane * e * cos(k·x)
         where:
           k = (kx, ky) from integer mode indices (nx, ny),
           e is a unit polarization vector with e·k = 0.
       - ex = ey = 0.
       - Clean single-k photon, ideal for dispersion tests.

Gauge sector implements genuine Maxwell dynamics in 2D with:
  Bz = d_x A_y - d_y A_x
  H_gauge = 1/2 E^2 + 1/2 c^2 Bz^2
...
def deriv_x(field, dx: float):
    """Central derivative w.r.t x with periodic BCs."""
    return (xp.roll(field, -1, axis=0) - xp.roll(field, +1, axis=0)) / (2.0 * dx)


def deriv_y(field, dx: float):
    """Central derivative w.r.t y with periodic BCs."""
...
def compute_Bz(ax, ay, dx: float):
    """
    Magnetic field in 2D:
      Bz = ∂x A_y - ∂y A_x
    """
    dAy_dx = deriv_x(ay, dx)
    dAx_dy = deriv_y(ax, dx)
...
class CoupledScalarMaxwell2D:
    def __init__(
        ...
        # FFT machinery for Poisson solver (defrag)
    ...
    def init_plane_wave(self, amp: float, nx: int, ny: int):
        """
        Seed a single transverse plane wave:

            A(x,y,0) = amp * e * cos(k·x)

        where:
          k = (kx, ky) from (nx, ny),
          e is a unit polarization vector with e·k = 0 (transverse gauge).
        ...
        self.ax = amp * ex_pol * cos_phase
        self.ay = amp * ey_pol * cos_phase
        self.ex[...] = 0.0
        self.ey[...] = 0.0
        self.psi[...] = 0.0 + 0.0j
        ...
        print(f"       polarization e=({ex_pol:.6f}, {ey_pol:.6f}) (transverse)")
    ...
    def total_energy(self) -> float:
        ...
        # Gauge electric + Maxwell magnetic energy
        E_g_el = 0.5 * xp.sum(self.ex ** 2 + self.ey ** 2)
        Bz = compute_Bz(self.ax, self.ay, self.dx)
        E_g_mag = 0.5 * (self.c ** 2) * xp.sum(Bz ** 2)
    ...
    def step_symplectic(self):
        ...
        # 1) First half-kick for gauge fields (E)
        jx, jy = gauge_current(self.psi, self.ax, self.ay, self.dx, self.q)
        Bz = compute_Bz(self.ax, self.ay, self.dx)
        dEx_dt = (self.c ** 2) * deriv_y(Bz, self.dx) - jx
        dEy_dt = -(self.c ** 2) * deriv_x(Bz, self.dx) - jy

        self.ex += 0.5 * dt * dEx_dt
        self.ey += 0.5 * dt * dEy_dt

        # 2) Drift for gauge coordinates (A)
        self.ax += dt * self.ex
        self.ay += dt * self.ey
        ...
        # 4) Second half-kick for gauge fields (E)
        jx, jy = gauge_current(self.psi, self.ax, self.ay, self.dx, self.q)
        Bz = compute_Bz(self.ax, self.ay, self.dx)
        dEx_dt = (self.c ** 2) * deriv_y(Bz, self.dx) - jx
        dEy_dt = -(self.c ** 2) * deriv_x(Bz, self.dx) - jy
...
def run_sim(args):
    ...
    if mode == "substrate":
        ...
        # Random gauge photons, no matter, no defrag
        sim.q = 0.0
        sim.g_defrag = 0.0
        ...
        # Clean plane-wave photon, no matter, no defrag
        sim.q = 0.0
        sim.g_defrag = 0.0
...
def main():
    parser = argparse.ArgumentParser(
        description="Coupled scalar + Maxwell gauge substrate with plane-wave photon mode."
    )
    ...
    parser.add_argument("--amp_plane", type=float, default=0.01,
                        help="Amplitude of plane-wave A.")
